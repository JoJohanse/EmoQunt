...
- **Data-source health beats** (`src/data/source_health.py`): each layer of the fetch chains in `data_manager.py` (stock & index, zh_a & us) calls `record_source_health('tushare'|'yfinance'|'sina'|'eastmoney'|'baostock', ok)` right where that source is actually attempted — sources that were never tried get no beats. Exposed at `GET /api/data/source-health` (in-process, last 7 attempts per source, reset on restart) and rendered as the homepage 数据源心跳 widget. When adding a new data source or chain layer, add a record call at its attempt site.
...
- **SPA local persistence layer**: `frontend/src/stores/persist.ts` is a zero-dependency Pinia plugin (opt-in via `defineStore(id, setupFn, { persist: true | { key?, pick?, revive? } })`), persisting state to `localStorage` under the `emoqunt:` prefix with silent try/catch degradation. Persisted stores: `ui` (theme/sidebar collapse/tourDone), `watchlist` (自选股 + last chart target), `backtestHistory` (回测摘要 + 上次表单, max 20 records), `chat` (messages, capped at 100, `streaming` flags stripped on revive), `favorites`/`homeLayout`/`tabs`. The Jinja2 backtest form remembers last inputs via inline script in `web/templates/backtest_form.html` (key `emoqunt:backtest_form`; URL/query preselection wins over localStorage).
