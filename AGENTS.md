...
- **A-share stock data: three-tier fallback** (`src/data/data_manager.py`): akshare `stock_zh_a_daily` (Sina, primary) → akshare `stock_zh_a_hist` (Eastmoney, fallback 1 — official-recommended, symbol is bare 6-digit code without sh/sz prefix) → baostock `query_history_k_data_plus` (fallback 2 — independent data server). Same pattern for indices: `stock_zh_index_daily` (Sina) → `stock_zh_index_daily_em` (Eastmoney, supports start/end) → baostock. baostock code format is `sh.600000` (dot-separated), dates are `YYYY-MM-DD`, `adjustflag` 1/2/3 means hfq/qfq/nfq (**reversed vs akshare**), volume unit is shares (code divides by 100 to match akshare's 手), and all numeric fields return as strings (code casts to float). `_BaoStockSession` manages login/logout with auto-relogin after 200 queries and exponential-backoff retries.
